...
def build_cert(subject_cn, issuer_obj, subject_key, issuer_key, *,
               ...
               subject_dn_extra=(),
               no_ski=False, ski_override=None, no_aki=False):
    """issuer_obj: certificate used to derive issuer Name + AKI (or None to
    self-issue). Returns an x509.Certificate.

    ``no_ski`` omits the subjectKeyIdentifier extension entirely (legacy
    issuer compatibility); ``ski_override`` sets an arbitrary SKI value even
    when the key is shared (same-key cross-signs with differing SKI);
    ``no_aki`` omits the authorityKeyIdentifier extension on the child.
    """
    ...
    if not no_ski:
        ski_value = ski_override if ski_override is not None else \
            x509.SubjectKeyIdentifier.from_public_key(subject_key.public_key()).digest
        builder = builder.add_extension(
            x509.SubjectKeyIdentifier(ski_value), critical=False)
