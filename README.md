...
# 期望最后一行：acceptance: 19/19 passed
```
...
- 节点 = 不同 DER（以 SHA-256 为身份）；交叉签发、重复对象、图环都是
  ...
  同一公钥被多个签发者交叉签发但 SKI 取值不同时，按公钥相同保留该候选，
  AKI 筛选不会丢失它。
...
- 规模友好：封存时用零密码学的原始 DER 抽取建立双重索引——按主体名，
  以及在每个主体名下再按 SKI、公钥（SHA-256 原始 subjectPublicKey
  位串）分组并登记缺少 SKI 的证书。裁决时**惰性解析**证书：子证书
  **携带 AKI** 时，只完整解析 SKI 匹配的签发者、无 SKI（须保留 RFC 5280
  兼容性）的候选，以及与这些候选**公钥相同**的交叉签发证书；同名但
  SKI 不同的干扰证书永不完整解析，处理量对干扰证书数量保持常数级。
  子证书**不含 AKI** 时仍然检查完整同名候选集合。100k 张无关证书的图中
  裁决少量叶证书，不会反复全量解析 DER。
...
- `package-manifest.json`：每个成员的 SHA-256/长度清单，并记录封存时
  签发者身份索引版本（`identity_index_version`；旧 v1 仅按主体名建桶的
  封存数据在离线复核时仍按完整同名集合扫描）；
...
app/                 服务
  ...
  loader.py          封存集惰性装载
  nameindex.py       封存时零密码学签发者身份索引（主体名/SKI/公钥，AKI 定域）
  package.py         确定性证据包构建
