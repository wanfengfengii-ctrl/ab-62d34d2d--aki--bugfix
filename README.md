...
- 规模友好：封存时用**零密码学**的 DER 抽取建立主体名 + SKI + 公钥摘要
  （identity v2）侧车索引；裁决时**惰性解析**证书——100k 张无关证书的图中
  裁决少量叶证书，不会反复全量解析 DER。
- **AKI 限定同名候选范围**：叶证书携带 AKI keyIdentifier 时，同名候选只保留
  SKI 匹配者、**缺少 SKI 的兼容性候选**，以及与 AKI 所指 SKI 持有者**同公钥**
  的候选（同钥交叉签发，即使其 SKI 不同或缺失也不丢弃）；完整解析数因此保持
  **常数级**，不随同名但 AKI 不匹配的干扰 CA 数量增长。叶证书**不带 AKI** 时，
  仍检查完整同名候选集合。在线裁决与离线复核对包内 DER 使用同一套侧车/内联
  构建逻辑，语义完全一致；仅有旧版主体名侧车（或没有任何侧车）的封存数据
  继续可用（旧库按 DER 惰性补算廉价身份）。
...
- 裁决中还含逐规则中间结论（rules）、逐层策略 trace、每个证书的撤销
  ...
  `final_digest`。`path_search.certificates_fully_parsed` 记录本裁决实际
  完整解析（密码学级）的证书数：叶带 AKI 时该值与同名干扰证书数量无关。
